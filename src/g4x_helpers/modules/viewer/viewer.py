...
def create_viewer_zarr(
    ...
    prepare_output(smp, out_dir, validator=ViewerZarr, overwrite=overwrite)

    if protein_list is not None:
        smp.proteins = protein_list
    ...
    write_images(smp, root_group)
    write_transcripts(smp, root_group, overwrite=True)

    prechew = prepare_cell_group_input(smp)
    write_cells(smp, root_group, prechew=prechew, overwrite=True)

    if smp.src.QCSummary.path_exists:
        shutil.copy(smp.tree.QCSummary.p, smp.tree.ViewerZarr.p / 'misc' / 'summary.html')
    else:
