...
def get_backend(prefer_gpu: bool = True) -> ComputeBackend:
    ...
    if prefer_gpu and rapids.available:
        import cupy as cp
        import rapids_singlecell as rsc

        return ComputeBackend(kind='gpu', rapids=rapids, cp=cp, rsc=rsc)

    return ComputeBackend(kind='cpu', rapids=rapids)


@lru_cache(maxsize=1)
def _get_rapids_backend():
    rapids = io.check_rapids()
...
def create_cell_x_protein(
    g4x_obj: 'G4Xoutput', mask: np.ndarray, signal_list: list[str] | None = None, suffix: str = '_intensity_mean'
) -> pl.LazyFrame:
    ...
    for signal_name in tqdm(signal_list, desc='Extracting protein signal'):
        ...
        use_gpu = _get_rapids_backend() is not None
        intensity_df = image_intensity_extraction(
            signal_img, mask_flat=mask_flat, bead_mask_flat=bead_mask_flat, ch_label=ch_label, rapids=use_gpu
        )
...
def image_intensity_extraction(
    ...
    backend = get_backend()
    if use_gpu and not backend.rapids.available:
        print('Warning: RAPIDS is not available, falling back to CPU implementation.')
        use_gpu = False

    if use_gpu:
        unique_labels, means = image_intensity_extraction_gpu(img, mask_flat, bead_mask_flat, backend=backend)
    else:
...
def optimize_leiden_clusters(
    adata, cluster_name='leiden_clusters', cluster_prefix='C', ideal_clusters=6, init_res=0.5, max_attempts=15
):
    step_sizes = {
        50: lambda x: x - (x * 0.5),
