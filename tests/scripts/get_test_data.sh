...
DATASET_ID="${1:-g4x_testdata_mini_v4.tar.gz}"
...
aws s3 cp "${S3_BUCKET_ROOT}/datasets/g4x_testdata/${DATASET_ID}" "datasets/${DATASET_ID}" --no-progress

# Clean up config and checksum file
mv "datasets/${DATASET_ID}" datasets/test_data.tar.gz
